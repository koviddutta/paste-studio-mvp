...
"""
# app/paste_core/base_templates.py
...
def _fetch_base_template_row(template_id: int) -> dict:
    ...
    resp = (
        supabase
        .table("base_formulation_templates")
        .select("*")
        .eq("id", template_id)
        .single()
        .execute()
...
def compute_base_template_from_db(template_id: int) -> BaseTemplateComposition:
    ...
    # Normalise parts to 100 g basis
    total_parts = sum(float(v) for v in ingredients_map.values())
    if total_parts <= 0:
        raise ValueError(f"base_formulation_templates id {template_id} has zero total parts.")
    ...
    for ing_name, parts in ingredients_map.items():
        fraction = float(parts) / total_parts  # fraction of base by weight
    ...
    # Placeholder AFP/POD/DE – refine later with sugar spectrum + gelato_science_constants
